...
1. 顶部导航栏目如何组织，尤其是“介绍”如何改为“关于协会”，以及“规章制度”如何并入“关于协会”二级栏目。
...
| F02 | 关于协会 | `/intro`，兼容 `/about` | 协会介绍、规章制度、组织架构、联系协会 |
| F03 | 关于协会 / 规章制度 | `/rules`，后续可规范为 `/about/rules` | 制度分类、制度列表、PDF、版本、生效日期；不作为顶部一级导航 |
...
- 悬停或展开“关于协会”展示二级栏目：协会介绍、规章制度、组织架构、联系协会。
- “规章制度”不作为顶部一级导航；`/rules` 仅作为历史路径或制度子页面路径保留。
- 鼠标悬停“发展中心”展示六个二级栏目。
- 当前路径对应的一级栏目高亮。
...
频道首页不能只是卡片堆砌或入口列表，而应同时承担正式说明页、内容聚合页和业务分流页。后台可维护页面标题、简介、Hero、图片、按钮、栏目导语、FAQ、说明文案、推荐内容、公告、制度、附件、页面区块、时间线、图文、下载列表、SEO、发布状态、版本记录和合规提示标准文案。CMS 不可随意配置申请表单字段校验、付款订单状态、证书 vt 访问规则、公开 DTO 字段范围、审核状态机流转、RBAC 权限逻辑、私有 Storage 文件路径、生产数据修改、删除、覆盖。
...
- `/rules` 短期保留为历史路径或制度子页面路径，后续可申请规范化到 `/about/rules`，但不出现在顶部一级导航。
- 修改受保护栏目路径或隐藏状态，需要二次确认并写入审计。
- 发展中心支持二级菜单管理。
- 关于协会支持二级栏目：协会介绍、规章制度、组织架构、联系协会。
