...
const navItems = [
  ...
  { href: "/exchange", label: "文化交流" },
  {
    ...
    label: "发展中心",
    children: [
      { href: "/development/health-practice", label: "养生实践发展中心" },
      { href: "/development/cultural-creative", label: "文化创意发展中心" },
      { href: "/development/education", label: "教育发展中心" },
      { href: "/development/international-exchange", label: "国际交流发展中心" },
      { href: "/development/yijing-cognition", label: "易学认知发展中心" },
      { href: "/development/dao-medicine", label: "道医中医文化发展中心" }
    ]
  },
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full max-w-full border-b border-[#d8d0bf] bg-[#f7f1e6]/98 shadow-[0_8px_22px_rgba(80,54,36,0.055)] backdrop-blur-md">
      ...
      <div className="mx-auto flex w-full max-w-[1440px] min-w-0 items-center gap-3 px-4 py-3 sm:px-8 xl:gap-5">
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          <BrandMark size="md" />
          <span className="min-w-0 max-w-[11rem] sm:max-w-[15rem] xl:max-w-[17rem]">
        ...
        <nav className="scrollbar-none hidden min-w-0 flex-1 items-center gap-1 overflow-x-auto overflow-y-visible border-x border-[#d8d0bf] bg-[#fbf8ef]/70 px-2 py-1 lg:flex">
          {navItems.map((item) => {
            ...
            return item.children ? (
              <div className="group relative" key={item.href}>
                <Link
                  className={`block whitespace-nowrap border-b-2 px-2.5 py-2 text-[13px] font-medium transition xl:px-3 ${
                    active
                ...
                <div className="invisible fixed left-1/2 top-[6.35rem] z-[70] w-80 -translate-x-1/2 pt-2 opacity-0 transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100">
              ...
              <Link
                className={`whitespace-nowrap border-b-2 px-2.5 py-2 text-[13px] font-medium transition xl:px-3 ${
                  active
                    ? "border-cinnabar text-cinnabar"
                    : "border-transparent text-[#33251F] hover:border-warmGold/60 hover:text-cinnabar"
                }`}
      ...
      <div className="w-full max-w-full overflow-hidden px-4 pb-3 sm:px-8 lg:hidden">
        <nav className="scrollbar-none flex max-w-full gap-1.5 overflow-x-auto border border-[#e4ded0] bg-white/82 p-1.5 shadow-[0_8px_20px_rgba(80,54,36,0.055)]">
          {navItems.map((item) => {
            const active = isActive(item.href, item.children);

            return item.children ? (
              <Link
                className={`shrink-0 rounded-lg px-3 py-2 text-center text-sm leading-5 transition sm:rounded-xl ${
